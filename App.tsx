...
const App: React.FC = () => {
  ...
  const selectStage = async (stage: UserStage) => {
    ...
    // Command to bot to start the sequence
    const systemTrigger = "Start the advisory session. Greet the user as NCPL Career Advisor and ask for their name.";
    ...
    if (interactionType === 'voice') {
      ...
      try {
        ...
        setMessages([{
          id: 'initial',
          role: 'model',
          text: "Hello! Welcome to NCPL Career Advisory. I'd love to help you find the right path. To get started, could you please tell me your name?",
          timestamp: new Date(),
          language: selectedLanguage
        }]);
      } finally {
  ...
  const connectVoice = useCallback(async (initialText: string) => {
    ...
    if (!apiKey) {
      setErrorMsg("Configuration error. Please check API settings.");
    ...
    const client = new LiveClient(apiKey, {
      ...
      onError: (err: any) => {
        handleVoiceFallback(`Connection issue. Switching to text mode.`);
      }
    });
  ...
  const handleSendMessage = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      setErrorMsg("I’m sorry, I encountered an issue. Please try again.");
    } finally {
      setIsProcessingText(false);
    }
  };
        ...
        <div className="flex items-center gap-3">
          <div className={`px-5 py-2.5 rounded-2xl flex items-center gap-3 transition-all shadow-sm border font-black text-[10px] uppercase tracking-[0.2em] ${
            ...
            <span className="hidden sm:inline">
              {interactionType === 'voice' ? 'Voice Advisor active' : 'Professional Chat active'}
            </span>
          </div>
        </div>
      </header>
      ...
      <main className="flex-1 flex flex-col md:flex-row overflow-hidden relative">
          ...
          <div className={`flex flex-col items-center justify-center p-8 transition-all duration-700 ${isConnected ? 'w-full md:w-[45%] bg-slate-50 border-r border-slate-100' : 'w-full'}`}>
            {isConnected && (
              <div className="mb-8 p-10 bg-white rounded-[4rem] shadow-2xl border border-slate-100 w-full max-w-sm animate-fade-in flex flex-col items-center glass-card relative overflow-hidden">
                 ...
                 <p className="text-[11px] font-black text-slate-400 uppercase tracking-[0.4em] mb-8">Live Global Audio</p>
            ...
            {!isConnected && (
              <div className="flex flex-col items-center gap-8 mb-12">
                ...
                <p className="text-slate-500 text-[11px] font-black uppercase tracking-[0.5em] animate-pulse">Establishing Advisor Link...</p>
              </div>
            )}
            ...
            <div className="mt-16 flex flex-col items-center gap-8 w-full">
              {isConnected && (
                <div className="flex flex-col items-center gap-4">
                   <button onClick={() => handleVoiceFallback("Voice session ended.")} className="w-24 h-24 rounded-full bg-gradient-to-br from-red-500 to-rose-600 flex items-center justify-center text-white shadow-2xl shadow-red-500/30 hover:scale-110 active:scale-95 transition-all group relative">
                    <PhoneOff size={36} />
                  </button>
                  <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Disconnect Session</span>
                </div>
              )}
        ...
        <div className="flex-1 flex flex-col overflow-hidden bg-white">
                    ...
                    {(msg.text.includes('NCPL') || msg.text.includes('roadmap') || msg.text.includes('Consulting')) && msg.role === 'model' && (
                      <div className="mt-8 p-5 bg-[#00AEEF]/5 rounded-[2rem] border border-[#00AEEF]/10 flex items-start gap-4 group">
                        ...
                        <div className="flex-1">
                          <p className="text-[11px] font-black text-[#00AEEF] uppercase tracking-widest mb-1">NCPL Program Advantage</p>
                          <a href="https://ncplconsulting.net" target="_blank" rel="noopener noreferrer" className="text-base font-bold text-slate-700 hover:text-[#00AEEF] hover:underline flex items-center gap-2 transition-all">
                            View Roadmap Details <ArrowRight size={16} />
                          </a>
                        </div>
                      </div>
                    )}
          ...
          <div className="p-10 md:p-14 border-t border-slate-100 bg-white sticky bottom-0 z-20 shadow-[0_-20px_60px_-20px_rgba(0,0,0,0.06)]">
            <form onSubmit={handleSendMessage} className="flex items-center gap-6 max-w-4xl mx-auto relative group">
              <div className="relative flex-1">
                <input 
                  ...
                  placeholder="Share your goals with the advisor..."
            ...
            <div className="mt-12 flex items-center justify-center gap-14 text-[11px] text-slate-400 font-black uppercase tracking-[0.5em] opacity-80">
              <span className="flex items-center gap-3"><CheckCircle2 size={18} className="text-emerald-500"/> Global Standard</span>
              <span className="flex items-center gap-3"><Lock size={18} className="text-[#00AEEF]"/> Secure Session</span>
              <span className="hidden lg:flex items-center gap-3"><Users size={18} className="text-amber-500"/> Verified Advisory</span>
            </div>
          </div>
        </div>
      </main>
